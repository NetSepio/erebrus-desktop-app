...
OUT_BASH_COMPLETION_SCRIPT=$OUT_DIR/erebrus.bash-completion
...
if [ ! -z "$EREBRUS_BUILD_SKIP_GLIBC_VER_CHECK" ] || [ ! -z "$GITHUB_ACTIONS" ]; 
then
  ...
  then
      ...
      echo "[ ] (you can define env var 'EREBRUS_BUILD_SKIP_GLIBC_VER_CHECK' to skip this check"
...
$SCRIPT_DIR/bash-completion-generator-erebrus-cli.sh "$OUT_FILE" > "$OUT_BASH_COMPLETION_SCRIPT"

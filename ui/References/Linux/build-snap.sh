...
EREBRUS_DESKTOP_UI2_SOURCES="$PROJECT_ROOT/ui"
...
[NOTES]
 ...
 To install the package, use command:
    ...
    Example:
        $ snap install erebrus_${VERSION}_amd64.snap --dangerous
 Steps required after install of manually! build snap:
    ...
    1) Manual connection of the required interfaces:
        $ sudo snap connect erebrus:network-control
        $ sudo snap connect erebrus:firewall-control
    2) Restart daemon:
        $ sudo snap restart erebrus.daemon

 To release/deploy package to SnapStore (only for Erebrus developers!):
    ...
    Example:
        $ snapcraft upload --release=beta erebrus_${VERSION}_amd64.snap
EOF

...
USER="${SUDO_USER:-$USER}"
UI_APP_USER_DIR="/home/${USER}/.config/Erebrus"
UI_APP_USER_DIR_OLD="/home/${USER}/.config/erebrus-ui" # (old productName='erebrus-ui')

AUTOSTART_FILE="/home/${USER}/.config/autostart/erebrus-ui.desktop"

DESKTOP_FILE_DIR=/usr/share/applications
DESKTOP_FILE=/usr/share/applications/Erebrus.desktop
...
echo "[+] Checking for 'erebrus-ui' running processes ..."
ps aux | grep /opt/erebrus/ui/bin/erebrus-ui | grep -v grep  > /dev/null 2>&1
if [ $? -eq 0 ]; then
  echo "[!] Detected: Erebrus app is running"

  # We should be careful here: WE SHOULD NOT KILL THIS SCRIPT :)
  # (which also can have 'erebrus-ui' in process description)
  silent kill -TERM $(ps aux | grep /opt/erebrus/ui/bin/erebrus-ui | grep -v grep | awk '{print $2}')
  silent sleep 2
  silent kill -KILL $(ps aux | grep /opt/erebrus/ui/bin/erebrus-ui | grep -v grep | awk '{print $2}')
fi
...
if [ "$1" = "upgrade" ] || [ "$1" = "1" ] ; then
  ...
  if [ -d $UI_APP_USER_DIR ] ; then
    ...
    # this is necessary for old application version (old productName='erebrus-ui')
...
# removing old application version cache (old productName='erebrus-ui')
if [ -d $UI_APP_USER_DIR_OLD ] ; then 

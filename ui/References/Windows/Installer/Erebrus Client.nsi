...
!define PRODUCT_NAME "Erebrus Client"
!define PRODUCT_IDENTIFIER "Erebrus Client"
!define PRODUCT_PUBLISHER "Erebrus Limited"

!define APP_RUN_PATH "$INSTDIR\ui\Erebrus Client.exe"
!define PROCESS_NAME "Erebrus Client.exe"
!define EREBRUS_SERVICE_NAME "Erebrus Client"
!define PATHDIR "$INSTDIR\cli"

!define DEVCON_BASENAME "devcon.exe"
!define PRODUCT_TAP_WIN_COMPONENT_ID "tapivpn"
;!define DRIVER_SPLIT_TUNNEL_ID "root\erebrus-split-tunnel"
...
!macro COMMON_INIT
  ...
  StrCpy $StartMenuFolder "Erebrus"
...
!define MUI_FINISHPAGE_RUN "$INSTDIR\Erebrus Client.exe"
...
CreateShortcut "$DESKTOP\Erebrus Client.lnk" "${APP_RUN_PATH}"
FunctionEnd
...
;===============================
; FINISH page modification handlers (add additional checkbox "Add Erebrus CLI binary to the path" to the 'finish' page)
Function fin_show
...
Function fin_pre
	...
	WriteINIStr "$PLUGINSDIR\iospecial.ini" "Field 6" "Text" "Add Erebrus CLI binary to the path"
...
Section "${PRODUCT_NAME}" SecIVPN
  ...
  ReadRegStr $R0 HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Erebrus Client" "UninstallString"
  StrCmp $R0 "" done
  ...
  ; Stop Erebrus service
  stopservcice:
  Call StopService
  Pop $0 ; 1 - SUCCESS;
  ${if} $0 != 1
		DetailPrint "ERROR: Failed to stop 'Erebrus Client' service."
		MessageBox MB_ABORTRETRYIGNORE|MB_ICONEXCLAMATION "Failed to stop 'Erebrus Client' service.$\nIgnoring this problem can lead to issues with Erebrus Client software in the future." IDRETRY stopservcice IDIGNORE ignoreservicestop
		DetailPrint "Aborted"
		Abort
  ${EndIf}
  ignoreservicestop:

  ; When service stopping - Erebrus Client must also Close automatically
  ; anyway, there could be situations when Erebrus Client not connected to service (cannot receive 'service exiting' notification.)
  ; Therefore, here we try to stop Erebrus Client process manually.
  ; Stop Erebrus Client application
  stopclient:
  Call StopClient
  Pop $0 ; 1 - SUCCESS
  ${if} $0 != 1
		DetailPrint "ERROR: Failed to stop 'Erebrus Client' application."
		MessageBox MB_ABORTRETRYIGNORE|MB_ICONEXCLAMATION "Failed to stop 'Erebrus Client' application.$\nIgnoring this problem can lead to issues with Erebrus Client software in the future." IDRETRY stopclient IDIGNORE ignoreclientstop
		DetailPrint "Aborted"
		Abort
  ${EndIf}
  ignoreclientstop:
  ...
  Push "$INSTDIR\Erebrus Firewall Native x64.dll" ; file to check for writting
  Push 15000 ; 15 seconds
  Call WaitFileOpenForWritting

  ; check is library can be overwritten
  Push "$INSTDIR\Erebrus Helpers Native x64.dll" ; file to check for writting
  Push 15000 ; 15 seconds
  Call WaitFileOpenForWritting
  ...
  ; "C:\Program Files\Erebrus Client\ui\Erebrus Client.exe" --hidden
  WriteRegStr    HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "${PRODUCT_IDENTIFIER}" '"${APP_RUN_PATH}" --hidden'
  reg_autostart_done:
  ; <<<

  ; extract all files from source dir (it is important that Erebrus Client Application must be stopped on this moment)
  File /r "${SOURCE_DIR}\*.*"
  ...
  CreateShortCut "$SMPROGRAMS\$StartMenuFolder\${PRODUCT_NAME}.lnk" "$INSTDIR\ui\Erebrus Client.exe"
...
*/
  ...
  /*
  ${If} ${AtLeastWin10}
    DetailPrint "Installing Erebrus Split-Tunnel Driver..."
    ...
    ${If} $R5 == 0
      ...
      nsExec::ExecToLog '"$INSTDIR\devcon\$BitDir\${DEVCON_BASENAME}" $R1 "$INSTDIR\SplitTunnelDriver\$BitDir\erebrus-split-tunnel.inf" ${DRIVER_SPLIT_TUNNEL_ID}'
      Pop $R0 # return value/error/timeout
  ...
  */
  ; ============ Service ======================================================================
  ; install service
  DetailPrint "Installing Erebrus Client service..."
  nsExec::ExecToLog '"$SYSDIR\sc.exe" create "Erebrus Client" binPath= "\"$INSTDIR\Erebrus Service.exe\"" start= auto'
  nsExec::ExecToLog '"$SYSDIR\sc.exe" sdset "Erebrus Client" "D:(A;;CCLCSWRPWPDTLOCRRC;;;SY)(A;;CCDCLCSWRPWPDTLOCRSDRCWDWO;;;BA)(A;;CCLCSWLOCRRC;;;IU)(A;;CCLCSWLOCRRC;;;SU)(A;;RPWPDTLO;;;S-1-1-0)"'

  ; add service to firewall
  ;nsExec::ExecToLog '"$SYSDIR\netsh.exe" firewall add allowedprogram "$INSTDIR\Erebrus Service.exe" "Erebrus Service" ENABLE'

  ; start service
  DetailPrint "Starting Erebrus Client service..."
  nsExec::ExecToLog '"$SYSDIR\sc.exe" start "Erebrus Client"'
SectionEnd
...
Section "Uninstall"
  ...
  nsExec::ExecToLog '"${PATHDIR}\erebrus.exe" disconnect'

  ${StrContains} $0 " -update" $CMDLINE
  ${If} $0 == ""
      ; uninstall
      DetailPrint "Ensure firewall is disabled..."
      nsExec::ExecToLog '"${PATHDIR}\erebrus.exe" firewall -persistent_off'
      nsExec::ExecToLog '"${PATHDIR}\erebrus.exe" firewall -off'

      DetailPrint "Logout..."
      nsExec::ExecToLog '"${PATHDIR}\erebrus.exe" logout'
  ${Else}
      ; update
  ${EndIf}

  ; stop service
  nsExec::ExecToLog '"$SYSDIR\sc.exe" stop "${EREBRUS_SERVICE_NAME}"'

  ; wait a little (give change for Erebrus Client application to stop)
  Sleep 1500
  ...
  ; remove service
  nsExec::ExecToLog '"$SYSDIR\sc.exe" delete "Erebrus Client"'

  ; removing firewall rules
  nsExec::ExecToLog '"$INSTDIR\erebruscli.exe" firewall disable'
  ...
  ; remove all
  Delete "$DESKTOP\Erebrus Client.lnk"
  ...
  ${If} $0 == ""
      ...
      RMDir /r "$APPDATA\Erebrus"
      SetShellVarContext all
      RMDir /r "$APPDATA\Erebrus"

      RMDir /r "$PROFILE\.erebrus" ; ~/.erebrus (created by CLI)      
  ${Else}
      ; update
  ${EndIf}
...
Function CheckOSSupported
    ${If} ${AtLeastWin10}
        goto archcheck
    ${EndIf}
    MessageBox MB_ICONSTOP|MB_OK "Unsupported Windows Version.$\nThis version of Erebrus Client can only be installed on Windows 10 and above."
    Quit
archcheck:
    ${If} ${RunningX64}
        goto end
    ${EndIf}
    MessageBox MB_ICONSTOP|MB_OK "Unsupported architecture.$\nThis version of Erebrus Client can only be installed on 64-bit Windows."
    Quit
end:
FunctionEnd
...
Function StopService
	DetailPrint "Checking is Erebrus Client service is running..."
	...
	DetailPrint "Stopping Erebrus Client service..."

	; stop service
	nsExec::ExecToStack '"$SYSDIR\sc.exe" stop "${EREBRUS_SERVICE_NAME}"'
	Pop $0 ; Return
	Pop $1 ; Output
	${If} $0 == '1060'
		DetailPrint "Erebrus Client service does not exist as an installed service [1060]"
		Push 1 		; Stopped OK
		Return
	${EndIf}
	${If} $0 != '0'
	...
	nsExec::ExecToStack '"$SYSDIR\taskkill" /fi "Services eq ${EREBRUS_SERVICE_NAME}" /F'
	Pop $0 ; Return
	Pop $1 ; Output
	${If} $0 < 0
...
Function IsServiceStopped
	nsExec::ExecToStack '"$SYSDIR\sc.exe" query "${EREBRUS_SERVICE_NAME}"'
	Pop $0 ; Return
	Pop $1 ; Output
	${If} $0 == '1060'
		DetailPrint "Erebrus Client service does not exist as an installed service [1060]"
...
; 	1 - Stopped (SECCUSS)
Function StopClient
	DetailPrint "Checking is Erebrus Client application is running..."
	...
	DetailPrint "Terminating Erebrus Client application..."
...
Function AddPath
	...
	${If} $1 == 0		; if received data is empty. Double-check it...		
		...
		${if} $2 > 0
			...
			MessageBox MB_OK|MB_ICONINFORMATION  "The PATH was not updated because the original length of characters exceeds the ${NSIS_MAX_STRLEN} character limit.$\r$\n$\r$\nTo facilitate quick access to the 'erebrus' command in the terminal, you can manually incorporate '${PATHDIR}' into the PATH variable within the Windows environment." /SD IDOK
			DetailPrint "The PATH was not updated because the original length of characters exceeds the ${NSIS_MAX_STRLEN} character limit.$\r$\n$\r$\nTo facilitate quick access to the 'erebrus' command in the terminal, you can manually incorporate '${PATHDIR}' into the PATH variable within the Windows environment."
			Goto end
		${EndIf}
	${EndIf}  

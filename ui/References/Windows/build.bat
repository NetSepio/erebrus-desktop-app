...
:build_service
	echo [*] Building Erebrus service and dependencies...
...
:copy_files
	...
	if NOT "%CERT_SHA1%" == "" (
		...
		echo.
		signtool.exe sign /tr %TIMESTAMP_SERVER% /td sha256 /fd sha256 /sha1 %CERT_SHA1% /v "%UI_BINARIES_FOLDER%\Erebrus.exe" || exit /b 1
		echo.
		echo Signing SUCCES
		echo.
	)
	...
	echo     Renaming UI binary to 'Erebrus Client.exe' ...
	rename  "%INSTALLER_TMP_DIR%\ui\Erebrus.exe" "Erebrus Client.exe" || goto :error
...
:build_installer
	...
	SET OUT_FILE="%INSTALLER_OUT_DIR%\Erebrus-Client-v%APPVER%.exe"
	%MAKENSIS% /DPRODUCT_VERSION=%APPVER% /DOUT_FILE=%OUT_FILE% /DSOURCE_DIR=%INSTALLER_TMP_DIR% "Erebrus Client.nsi"
	IF not ERRORLEVEL 0 (
...
:error
	goto :remove_tmp_vars_before_exit
	echo [!] Erebrus Client installer build FAILED with error #%errorlevel%.
	exit /b %errorlevel%

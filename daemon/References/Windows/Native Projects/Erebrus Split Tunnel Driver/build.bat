...
echo [ ] The scipt is building Erebrus Split-Tunnelling Driver
echo [ ] and preparing a CAB file to be send to Microsoft Partner portal for Attestation signing.
echo.
rem The scipt is building Erebrus Split-Tunnelling Driver
...
set RELEASE_PATH=%SCRIPTDIR%erebrus-split-tunnel\x64\Release\erebrus-split-tunnel
...
echo [+] Building ...
msbuild "%SCRIPTDIR%erebrus-split-tunnel\erebrus-split-tunnel.vcxproj" /p:Configuration=Release /p:Platform=x64 /p:SignMode=Off || goto :error
...
copy "%RELEASE_PATH%\erebrus-split-tunnel.inf" "%DIST_PATH_DRV%\erebrus-split-tunnel.inf" || goto :error
copy "%RELEASE_PATH%\erebrus-split-tunnel.sys" "%DIST_PATH_DRV%\erebrus-split-tunnel.sys" || goto :error
copy "%RELEASE_PATH%\WdfCoinstaller01009.dll" "%DIST_PATH_DRV%\WdfCoinstaller01009.dll" || goto :error
copy "%RELEASE_PATH%\..\erebrus-split-tunnel.pdb" "%DIST_PATH_DRV%\erebrus-split-tunnel.pdb" || goto :error

rem echo [+] Signing SYS file by EV Certificate ...
rem set TIMESTAMP_SERVER=http://timestamp.digicert.com
rem signtool sign /tr %TIMESTAMP_SERVER% /td sha256 /fd sha256 /sha1 %CERT_SHA1% /v "%DIST_PATH_DRV%\erebrus-split-tunnel.sys" || goto :error

echo [+] Preparing CAB file configuration...
>"%DIST_PATH_OUT%\erebrus-split-tunnel.ddf" (
  ...
  echo .Set CabinetNameTemplate=erebrus-split-tunnel.cab
  echo .Set DestinationDir=Package
  echo .Set DiskDirectoryTemplate="%DIST_PATH_OUT%"
  echo "%DIST_PATH_DRV%\erebrus-split-tunnel.inf"
  echo "%DIST_PATH_DRV%\erebrus-split-tunnel.sys"
  echo "%DIST_PATH_DRV%\WdfCoinstaller01009.dll"
	echo "%DIST_PATH_DRV%\erebrus-split-tunnel.pdb"
)
IF %ERRORLEVEL% NEQ 0 goto error

echo [+] Generating CAB file ...
cd "%DIST_PATH_OUT%"
makecab /f "%DIST_PATH_OUT%\erebrus-split-tunnel.ddf" || goto :error
...
signtool sign /fd sha256 /sha1 %CERT_SHA1% /v "%DIST_PATH_OUT%\erebrus-split-tunnel.cab" || goto :error

echo.
echo [ ] Erebrus Split-Tunnelling Driver: Build SUCCESS
echo [ ] CAB file: %DIST_PATH_OUT%\erebrus-split-tunnel.cab
echo.
...
:error
echo.
echo [!] FAILED building Erebrus Split-Tunnelling Driver. Error #%errorlevel%.
echo.
exit /b %errorlevel%

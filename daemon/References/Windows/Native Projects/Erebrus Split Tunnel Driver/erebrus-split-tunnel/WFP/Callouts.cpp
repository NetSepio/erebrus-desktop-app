...
namespace wfp
{
	...
	// (e.g. to bypass Erebrus firewall default blocking rule)
	...
	)
	{
		...
		mCallout.providerKey = const_cast<GUID*>(&KEY_EREBRUS_ST_PROVIDER);
	...
	)
	{
		...
		auto status = AddAndRegisterCallout(wdfDevObject, wfpEngineHandle,
			...
			&FWPM_LAYER_ALE_BIND_REDIRECT_V4,
			L"Erebrus Callout for split tunnelling (BIND_REDIRECT_V4)", NULL);
		if (!NT_SUCCESS(status))
			return status;
		...
		status = AddAndRegisterCallout(wdfDevObject,	wfpEngineHandle,
			...
			&FWPM_LAYER_ALE_CONNECT_REDIRECT_V4,
			L"Erebrus Callout for split tunnelling (CONNECT_REDIRECT_V4)", NULL);
		if (!NT_SUCCESS(status))
			return status;
		...
		status = AddAndRegisterCallout(wdfDevObject, wfpEngineHandle,
			...
			&FWPM_LAYER_ALE_BIND_REDIRECT_V6,
			L"Erebrus Callout for split tunnelling (BIND_REDIRECT_V6)", NULL);
		if (!NT_SUCCESS(status))
			return status;
		...
		status = AddAndRegisterCallout(wdfDevObject, wfpEngineHandle,
			...
			&FWPM_LAYER_ALE_CONNECT_REDIRECT_V6,
			L"Erebrus Callout for split tunnelling (CONNECT_REDIRECT_V6)", NULL);
		if (!NT_SUCCESS(status))
			return status;
		...
		status = AddAndRegisterCallout(wdfDevObject, wfpEngineHandle,
			...
			&FWPM_LAYER_ALE_AUTH_CONNECT_V4,
			L"Erebrus Callout for split tunnelling (ALE_AUTH_CONNECT_V4)", NULL);
		if (!NT_SUCCESS(status))
			return status;
		...
		status = AddAndRegisterCallout(wdfDevObject, wfpEngineHandle,
			...
			&FWPM_LAYER_ALE_AUTH_CONNECT_V6,
			L"Erebrus Callout for split tunnelling (ALE_AUTH_CONNECT_V6)", NULL);
		if (!NT_SUCCESS(status))
			return status;
		...
		status = AddAndRegisterCallout(wdfDevObject, wfpEngineHandle,
			...
			&FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4,
			L"Erebrus Callout for split tunnelling (ALE_AUTH_RECV_ACCEPT_V4)", NULL);
		if (!NT_SUCCESS(status))
			return status;
		...
		status = AddAndRegisterCallout(wdfDevObject, wfpEngineHandle,
			...
			&FWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6,
			L"Erebrus Callout for split tunnelling (ALE_AUTH_RECV_ACCEPT_V6)", NULL);
		if (!NT_SUCCESS(status))
			return status;

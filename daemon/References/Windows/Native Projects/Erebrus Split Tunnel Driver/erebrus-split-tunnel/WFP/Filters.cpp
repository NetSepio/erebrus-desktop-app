...
namespace wfp
{
		...
		const GUID filterKey, const GUID layerKey, const GUID calloutKey)
	{
		...
		filter.providerKey = const_cast<GUID*>(&KEY_EREBRUS_ST_PROVIDER);
		filter.layerKey = layerKey;
		filter.subLayerKey = KEY_EREBRUS_ST_SUBLAYER;
	...
	NTSTATUS RegisterFilters(HANDLE wfpEngineHandle)
	{
		...
		status = AddCalloutFilter(wfpEngineHandle, 
			L"Erebrus Split Tunnel filter (BIND_REDIRECT_V4)", NULL,
		...
		// CONNECT_REDIRECT_V4
		status = AddCalloutFilter(wfpEngineHandle,
			L"Erebrus Split Tunnel filter (CONNECT_REDIRECT_V4)", NULL,
		...
		// BIND_REDIRECT_V6
		status = AddCalloutFilter(wfpEngineHandle,
			L"Erebrus Split Tunnel filter (BIND_REDIRECT_V6)", NULL,
		...
		// CONNECT_REDIRECT_V6
		status = AddCalloutFilter(wfpEngineHandle,
			L"Erebrus Split Tunnel filter (CONNECT_REDIRECT_V6)", NULL,
		...
		// ALE_AUTH_CONNECT_V4
		status = AddCalloutFilter(wfpEngineHandle,
			L"Erebrus Split Tunnel filter (ALE_AUTH_CONNECT_V4)", NULL,
		...
		// ALE_AUTH_CONNECT_V6
		status = AddCalloutFilter(wfpEngineHandle,
			L"Erebrus Split Tunnel filter (ALE_AUTH_CONNECT_V6)", NULL,
		...
		status = AddCalloutFilter(wfpEngineHandle,
			L"Erebrus Split Tunnel filter (ALE_AUTH_RECV_ACCEPT_V4)", NULL,
		...
		status = AddCalloutFilter(wfpEngineHandle,
			L"Erebrus Split Tunnel filter (ALE_AUTH_RECV_ACCEPT_V6)", NULL,

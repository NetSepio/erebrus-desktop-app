...
namespace wfp
{
	...
	NTSTATUS Start(WDFDEVICE wdfDevice) 
	{
		...
		provider.providerKey = KEY_EREBRUS_ST_PROVIDER;
		provider.displayData.name = const_cast<wchar_t*>(L"Erebrus Split Tunnel");
		provider.displayData.description = const_cast<wchar_t*>(L"Erebrus Split Tunnel filters + callouts");
		...
		subLayer.subLayerKey = KEY_EREBRUS_ST_SUBLAYER;
		subLayer.displayData.name = L"Erebrus Split Tunnel sub-Layer";
		subLayer.displayData.description = L"Erebrus Split Tunnel sub-Layer for use callouts";
		subLayer.flags = 0;
